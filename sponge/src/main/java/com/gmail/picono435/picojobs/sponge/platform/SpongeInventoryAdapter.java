...
public class SpongeInventoryAdapter implements InventoryAdapter {
    ...
    public void create(String title, int size) {
        ...
        this.title = title;
        this.size = size;
        this.viewableInventory = ViewableInventory.builder().type(containerType).completeStructure().build();
    }

    @Override
    public void setItem(int slot, ItemAdapter item) {
        this.viewableInventory.set(slot, toItemStack(item));
    }

    @Override
    public void setItem(int slot, ItemAdapter item, ClickAction clickAction) {
        ItemStack itemStack = toItemStack(item);
        this.viewableInventory.set(slot, itemStack);
        InventoryMenuListener.actionItems.put(itemStack, clickAction);
    }

    @Override
    public ItemAdapter getItem(int slot) {
        return toItemAdapter(this.viewableInventory.peekAt(slot).get());
    }

    @Override
    public boolean isEmpty(int slot) {
        return this.viewableInventory.slot(slot).get().peek() == ItemStack.empty();
    }
    ...
    public ItemStack toItemStack(ItemAdapter itemAdapter) {
        ItemType itemType = ItemTypes.registry().value(ResourceKey.resolve(itemAdapter.getMaterial()));
        ...
        if(itemAdapter.getName() != null) itemStack.offer(Keys.DISPLAY_NAME, LegacyComponentSerializer.legacyAmpersand().deserialize(itemAdapter.getName()));
    ...
    public ItemAdapter toItemAdapter(ItemStack itemStack) {
        ...
        if(itemStack.getKeys().contains(Keys.DISPLAY_NAME)) itemAdapter.setName(LegacyComponentSerializer.legacyAmpersand().serialize(itemStack.get(Keys.DISPLAY_NAME).get()));
    ...
    public ViewableInventory getViewableInventory() {
        return viewableInventory;
    }
}
