...
public class FillListener implements PlayerEvent.FillBucket {
    ...
    public CompoundEventResult<ItemStack> fill(Player player, Level level, ItemStack stack, @Nullable HitResult target) {
        ...
        Job job = jp.getJob();
        if(!job.getTypes().contains(Type.FILL)) return CompoundEventResult.pass();
        if(!jp.isInWorkZone(player.getUUID())) return CompoundEventResult.pass();

        if(!job.inWhitelist(Type.FILL, level.getBlockState(((BlockHitResult) target).getBlockPos()).getBlock())) return CompoundEventResult.pass();
